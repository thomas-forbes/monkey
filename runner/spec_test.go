	...
	groups := []specGroup{
		...
		},
		{
			...
			cases: []specCase{
				...
				{name: "forEach", input: `let a = [1, 2, 3]; let mut sum = 0; forEach(a, fn(x) { sum = sum + x }); sum;`, expected: 6},
				...
				{name: "some true", input: `let a = [1, 2, 3]; let even = fn(x) { x % 2 == 0 }; some(a, even);`, expected: true},
				{name: "some false", input: `let a = [1, 3, 5]; let even = fn(x) { x % 2 == 0 }; some(a, even);`, expected: false},
				{name: "every true", input: `let a = [2, 4, 6]; let even = fn(x) { x % 2 == 0 }; every(a, even);`, expected: true},
				{name: "every false", input: `let a = [1, 4, 6]; let even = fn(x) { x % 2 == 0 }; every(a, even);`, expected: false},
			},
		},
		{
			name: "std math",
			cases: []specCase{
				{name: "max", input: `max(1, 5)`, expected: 5},
				{name: "min", input: `min(1, 5)`, expected: 1},
				{name: "equal min max", input: `min(1, 1) + max(1, 1)`, expected: 2},
				{name: "abs", input: `abs(-5)`, expected: 5},
				{name: "abs zero", input: `abs(0)`, expected: 0},
				{name: "clamp noop", input: `clamp(5, 1, 10)`, expected: 5},
				{name: "clamp lower", input: `clamp(0, 1, 10)`, expected: 1},
				{name: "clamp upper", input: `clamp(15, 1, 10)`, expected: 10},
				{name: "pow", input: `pow(5, 5)`, expected: 625},
				{name: "pow", input: `pow(5, 0)`, expected: 1},
				{name: "pow", input: `pow(5, 1)`, expected: 5},
			},
		},
	}
